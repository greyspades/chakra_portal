...
export const JobList = ({ roles, setRole, apply, getRoles, currentStep, refresh, count }: JobListProps) => {
      ...
      const parseSkills = (skills: string) => {
        if(skills) {
          return JSON.parse(skills).map((item: string, idx: number) => (
            <div key={idx} className="text-black border-2 border-green-700 border-solid rounded-lg px-2 py-1">
              {item}
            </div>
          ))
        }
      ...
      // if(item.status == "active") {
        ...
        <Paper className={currentStep == 1 ? "bg-white p-2 md:px-8 px-2" : "bg-white p-2 md:px-4 px-2"}>
            ...
            <div className="w-[100%] justify-between flex flex-row md:px-0 px-2">
              <div className={currentStep == 1 ? "md:w-[50%] w-[100%] flex flex-row gap-8" : "w-[150%] flex flex-row gap-2"}>
  ...
  return <div className="text-[14px] md:text-[16px]">
    <div className="">{renderRoles()}</div>
    <div className="flex justify-center bg-white h-[60px] rounded-md flex-row">
    <div className={currentStep == 1 ? "flex flex-row md:w-[50%] w-[80%] justify-between place-items-center" : "flex flex-row w-[100%] justify-between place-items-center"}>
    ...
    <IconButton className="ml-[30px] rounded-full shadow-md" onClick={refresh}>
